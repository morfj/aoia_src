...
LRESULT InventoryView::OnCopyItemRef(WORD FromAccelerator, WORD CommandId, HWND hWndCtrl, BOOL& bHandled)
{
    ...
    TCHAR buffer[1024];
    if (CommandId == ID_COPY_AUNO_ITEMREF || CommandId == ID_COPY_AUNO_ITEMREF_VBB) {
        ATL::AtlLoadString(IDS_AUNO_ITEMREF_URL, buffer, 1024);
    }
    else if (CommandId == ID_COPY_JAYDEE_ITEMREF || CommandId == ID_COPY_JAYDEE_ITEMREF_VBB) {
    ...
    switch (CommandId) {
        ...
        case ID_COPY_JAYDEE_ITEMREF:
            clip << _T("<a href=\"") << url << _T("\">") << pItemInfo->itemname << _T("</a> [QL") << pItemInfo->itemql << _T("] [") << itemlocation << _T("]");
            break;

        case ID_COPY_AUNO_ITEMREF_VBB:
        case ID_COPY_JAYDEE_ITEMREF_VBB:
            clip << _T("[url=") << url << _T("]") << pItemInfo->itemname << _T("[/url] {QL") << pItemInfo->itemql << _T("} {") << itemlocation << _T("}");
            break;

        case ID_COPY_AO_ITEMREF:
            clip << _T("<a href=\"text://<a href=itemref://") << pItemInfo->itemloid << _T("/") << pItemInfo->itemhiid << _T("/") 
                << pItemInfo->itemql << _T(">") << pItemInfo->itemname << _T("</a> [QL") << pItemInfo->itemql << _T("] [") << itemlocation 
                << _T("]<p><p><center>Generated by <a href='chatcmd:///start http://ia.frellu.net'>AOIA</a></center>\">") 
                << pItemInfo->itemname << _T("</a>");
            break;

        case ID_VIEW_ITEMSTATS_AUNO:
            break;

        case ID_VIEW_ITEMSTATS_JAYDEE:
            break;
